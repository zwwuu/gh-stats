      ...
      {!data && !error && (
        <Container maxWidth={"container.xl"} py={6}>
          <Stack align={"center"} spacing={4}>
            ...
            <Text>
              Generating stats for <Text as={"span"} fontWeight="semibold">{`${owner}/${repo}`}</Text>
            </Text>
          </Stack>
        </Container>
      )}
              ...
              <VStack align={"start"} spacing={4}>
                <Flex gap={4} justify={"space-between"} width={"100%"} wrap={"wrap"}>
                  <Heading as={"h1"} wordBreak={"break-all"}>
                    ...
                    <Text as={"span"} color={separatorColor}>
                      {" / "}
                    </Text>
                  ...
                  <Flex align="center" gap={4} wrap={"wrap"}>
                    ...
                    <Tag colorScheme="teal" size={"md"} variant={"outline"}>
                      ...
                      <TagLabel>
                        {"Fork "}
                        <Text as={"span"} fontWeight={"semibold"}>
                          {new Intl.NumberFormat(undefined, { notation: "compact" }).format(data.repoData.forks_count)}
                        </Text>
                      </TagLabel>
                    </Tag>
                    <Tag colorScheme="teal" size={"md"} variant={"outline"}>
                ...
                {data.repoData.description && <Text>{data.repoData.description}</Text>}
